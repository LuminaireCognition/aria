...
class Credentials:
    ...
    def resolve(cls, project_dir: Optional[Path] = None) -> Optional["Credentials"]:
        ...
        # Priority 3: Numeric credential files in directory
        if creds_dir.exists():
            _debug_log(f"Scanning credentials directory: {creds_dir}")
            cred_files = [
                p
                for p in sorted(creds_dir.glob("*.json"))
                if p.is_file() and re.fullmatch(r"[0-9]+", p.stem)
            ]
            if len(cred_files) > 1:
                files = "\n".join(f"  {p.name}" for p in cred_files)
                raise CredentialsError(
    ...
    def refresh_if_needed(self, script_dir: Optional[Path] = None) -> bool:
        ...
        if script_dir is None:
            ...
            if project_dir is None:
                _debug_log("Cannot refresh: project directory not found")
                return False
            script_dir = project_dir / ".claude" / "scripts"

        refresh_script = script_dir / "aria-token-refresh.py"
        if not refresh_script.exists():
            _debug_log(f"Token refresh script not found: {refresh_script}")
            return False
        ...
        try:
            result = subprocess.run(
                ["uv", "run", "python", str(refresh_script), "--quiet"],
                capture_output=True,
                timeout=30,
            )
            if result.returncode == 2:
                _debug_log("Token refresh failed with blocking error (exit code 2)")
            elif result.returncode != 0:
                _debug_log(f"Token refresh exited with code {result.returncode}")
...
def get_authenticated_client() -> tuple["ESIClient", Credentials]:
    ...
    if creds.token_expiry:
        try:
            ...
            pass  # Can't parse expiry, proceed and let ESI respond
...
def get_pilot_directory(project_dir: Optional[Path] = None) -> Optional[Path]:
    ...
    if pilots_dir.exists():
        matches = [
            d for d in pilots_dir.iterdir() if d.is_dir() and d.name.startswith(f"{pilot_id}_")
        ]
        if len(matches) > 1:
            dirs = "\n".join(f"  {d.name}" for d in sorted(matches))
            raise CredentialsError(
                f"Multiple pilot directories found for {pilot_id}:\n{dirs}\n"
