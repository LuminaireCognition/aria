...
class TankSelectionResult:
    ...
    selection_reason: str = (
        ""  # "armor_skills_higher", "shield_skills_higher", "tie_breaker", "default", "override"
    )
...
def select_tank_variant(
    ...
    discovered_paths = [path for path in (available_variant_paths or []) if path in valid_paths]

    available_variants = list(config.available)
    if discovered_paths:
        available_variants = [
            variant for variant in config.available if _variant_to_path(variant) in discovered_paths
        ]
        if not available_variants:
            available_variants = list(discovered_paths)
    ...
    if "shield" in available_paths:
        shield_score, shield_levels = calculate_tank_score(
            shield_skills, pilot_skills, shield_weight
        )
    else:
