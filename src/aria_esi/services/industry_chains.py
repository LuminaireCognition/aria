...
def _load_terminal_materials() -> dict[str, Any]:
    ...
    ref_path = (
        Path(__file__).parent.parent.parent / "reference" / "industry" / "terminal_materials.json"
    )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "savings_percent": round(
                (self.savings / self.total_market_cost * 100) if self.total_market_cost > 0 else 0,
                1,
            ),
...
class ChainResolver:
    ...
    def resolve(self, product_name: str) -> ChainResolutionResult:
        ...
        if not blueprint_info or "error" in blueprint_info:
            return self._make_terminal_result(
                product_name, blueprint_info.get("error", "No blueprint found")
            )
    ...
    def _resolve_node(
        ...
        if depth > self._max_depth:
            self._warnings.append(f"Max depth ({self._max_depth}) reached at {type_name}")
            return ChainNode(
                type_name=type_name,
                type_id=type_id,
                quantity=quantity,
    ...
    def _make_terminal_result(self, product_name: str, reason: str) -> ChainResolutionResult:
