...
class SovereigntyDatabase:
    ...
    def get_sovereignty_batch(self, system_ids: Sequence[int]) -> dict[int, SovereigntyRecord]:
        """
    ...
    def get_alliances_batch(self, alliance_ids: Sequence[int]) -> dict[int, AllianceRecord]:
    ...
    def get_coalition_by_alias(self, alias: str) -> CoalitionRecord | None:
        ...
        rows = conn.execute("SELECT * FROM coalitions").fetchall()
    ...
    def save_coalition_members(self, coalition_id: str, alliance_ids: Sequence[int]) -> int:
        """
    ...
    def get_stats(self) -> dict:
        ...
        # Get freshness
        sov_oldest = conn.execute("SELECT MIN(updated_at) FROM sovereignty_map").fetchone()[0]
        sov_newest = conn.execute("SELECT MAX(updated_at) FROM sovereignty_map").fetchone()[0]
