...
class PlanetCacheService:
    ...
    def save_cache(self) -> None:
        ...
        total_planets = sum(len(s.get("planets", [])) for s in systems.values())
    ...
    def find_systems_with_planet_types(
        ...
        for name, data in systems.items():
            ...
            types_in_system = {
                p.get("type_name", "").lower() for p in planets if p.get("type_name")
            }
...
async def fetch_system_planets(
    ...
    for planet_entry in planet_ids:
        planet_id = (
            planet_entry.get("planet_id") if isinstance(planet_entry, dict) else planet_entry
        )
