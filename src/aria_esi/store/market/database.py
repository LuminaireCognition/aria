...
class MarketDatabase:
    ...
    def _initialize_schema(self) -> None:
        ...
        conn.executescript(SCHEMA_SQL.format(schema_version=SCHEMA_VERSION))
        conn.commit()
    ...
    def _get_schema_version(self) -> int:
        ...
        try:
            ...
            # metadata table does not exist yet, or stored value is corrupt
            return 0
    ...
    def _run_migrations(self, from_version: int) -> None:
        ...
        if from_version < 10:
            logger.info("Running migration 9 -> 10: Case-insensitive watchlist names")
            # Step 1: Dedup merge — re-point items from case-variant duplicates to survivor
            dupes = conn.execute("""
                SELECT MIN(watchlist_id) as survivor_id, GROUP_CONCAT(watchlist_id) as all_ids
                FROM entity_watchlists
                GROUP BY name COLLATE NOCASE, owner_character_id
                HAVING COUNT(*) > 1
            """).fetchall()
            for row in dupes:
                survivor_id = row[0]
                all_ids = [int(x) for x in row[1].split(",")]
                duplicate_ids = [x for x in all_ids if x != survivor_id]
                for dup_id in duplicate_ids:
                    conn.execute(
                        """
                        INSERT OR IGNORE INTO entity_watchlist_items
                            (watchlist_id, entity_id, entity_type, entity_name, added_at, added_reason)
                        SELECT ?, entity_id, entity_type, entity_name, added_at, added_reason
                        FROM entity_watchlist_items WHERE watchlist_id = ?
                    """,
                        (survivor_id, dup_id),
                    )
                    conn.execute("DELETE FROM entity_watchlists WHERE watchlist_id = ?", (dup_id,))
            # Step 2: Rebuild indexes with COLLATE NOCASE
            conn.executescript("""
                ...
                CREATE UNIQUE INDEX idx_entity_watchlists_global
                    ON entity_watchlists(name COLLATE NOCASE) WHERE owner_character_id IS NULL;
            ...
            """)
            conn.commit()
            logger.info("Migration 9 -> 10 complete")
