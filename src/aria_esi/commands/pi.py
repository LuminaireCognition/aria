...
def cmd_pi_near(args: argparse.Namespace) -> dict:
    ...
    for system_name, system_data in systems_data.items():
        ...
        useful_types = (
            planet_types_in_system & set(single_planet_options) if single_planet_options else set()
        )
    ...
    filtered_matches.sort(
        key=lambda x: (
            ...
            x.get("distance_jumps")
            if x.get("distance_jumps") is not None
            else 9999,  # Then by distance
...
def _load_home_systems() -> list[str]:
    ...
    try:
        ...
        return [s["name"] for s in systems if s.get("classification") == "home"]
