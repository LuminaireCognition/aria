...
def cmd_sov_update(args: argparse.Namespace) -> dict:
    ...
    try:
        ...
        alliance_ids = list({r.alliance_id for r in records if r.alliance_id is not None})
    ...
    try:
        ...
        if sov is None:
            ...
            is_wormhole = (
                (
                    system_name
                    and system_name.startswith("J")
                    and len(system_name) > 1
                    and system_name[1:].replace("-", "").isdigit()
                )
        ...
        if not skip_validation:
            ...
            if not validation_result.get("valid", False):
                ...
                if validation_result.get("duplicate_count", 0) > 0:
                    ...
                    hints.append(
                        "Fix duplicates manually - each alliance can only belong to one coalition"
                    )
            ...
            logger.info(
                "Validation passed: %d alliances verified", validation_result.get("valid_count", 0)
            )
            ...
            if esi_name != alliance_name:
                ...
                invalid_entries.append(
                    (
                        ...
                        alliance_name,
                        alliance_id,
                        f"Name mismatch: ESI says '{esi_name}'",
                    )
                )
            else:
