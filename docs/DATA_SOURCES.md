...
> **Audience:** This document serves dual purposes. It is an **ARIA runtime policy** (the agent follows these rules when fetching external data) and a **contributor reference** for anyone adding or verifying non-SDE game data. Tool call syntax like `sde(action="...")` and `WebFetch(...)` are instructions for the ARIA agent, not CLI commands.
...
|-----------|---------|---------|--------|
| Item stats, descriptions | Yes | - | `sde(action="item_info")` |
| Blueprint materials | Yes | - | `sde(action="blueprint_info")` |
| Skill attributes | Yes | - | `sde(action="skill_requirements")` |
| NPC agent locations | Yes | - | `sde(action="agent_search")` |
| Market prices | - | Yes | `market(action="prices")` |
...
```
sde(action="agent_search", corporation, level, division) → universe(action="route") for distances → Sort by jumps
```
...
```
sde(action="agent_search", corporation, highsec_only=True) → Group by level
```
Example: "Show me SOE agents in high-sec only"

**4. Find agents in [System] for [Corporation]**
```
sde(action="agent_search", corporation, system="Arnon")
```
...
**Note:** NPC agent locations are now in SDE via `sde(action="agent_search")`. DOTLAN is no longer the primary source.
