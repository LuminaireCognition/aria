...
| Command | Description | Example |
|---------|-------------|---------|
| `/arbitrage` | Cross-region arbitrage opportunity scanner. Find profitable trade routes between trade hubs with hauling score analysis. | "arbitrage opportunities" |
| `/assets` | Asset inventory with valuation. View assets across stations with market value calculations. | "my assets" |
| `/contracts` | Personal contract management. View item exchange, courier, and auction contracts - both issued and received. | "my contracts" |
| `/find` | Find market sources near your location. Use for finding blueprints, items, or specific market sources by proximity. | "find [item] near me" |
...
| `/journal` | Log mission completions and exploration discoveries to operational records. | "log mission" |
| `/mail` | Read EVE mail headers and bodies. View inbox, filter unread, and read specific messages. | "my mail" |
