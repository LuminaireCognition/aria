...
Cleanup runs automatically on poller startup.
...
1. Test webhook: `uv run aria-esi notifications test <profile-name>`
...
- **Discord webhook URLs are bearer credentials**—keep `userdata/notifications/*.yaml` private (and protect all `userdata/` files)
...
- `dev/docs/ai-runtime/SESSION_BEHAVIOR.md` - Pilot data file reference and session behavior
