...
> **Audience:** This is an **ARIA runtime policy** — it defines rules the ARIA agent must follow when presenting game data. Users may find it useful for understanding how ARIA validates information and why you can trust its output. Contributors can use it as a reference for verification standards. Tool call syntax like `sde(action="...")` and `fitting(action="...")` are instructions for the agent, not CLI commands.
...
| 2 | **Pyfa/EOS** | `fitting(action="calculate_stats")` | Fitting calculations, DPS, EHP, capacitor |
...
```
User: "What does Drones skill do?"
ARIA: [Calls sde(action="item_info", item="Drones")]
...
```
User: "How much DPS does a Vexor do?"
ARIA: [Calls fitting(action="calculate_stats") with actual fit]
...
```
User: "What do I need for Hammerhead II?"
ARIA: [Calls skills(action="t2_requirements", item="Hammerhead II")]
...
ARIA: [Calls sde(action="blueprint_info"), gets manufacturing data but no invention data]
...
```
sde(action="item_info", item=...)           # Item description, stats, category
sde(action="skill_requirements", item=...)  # Full skill tree for ships/modules
sde(action="blueprint_info", item=...)      # Manufacturing data, materials
sde(action="search", query=...)             # Find items by partial name
```

### Skill Planning Tools
```
skills(action="easy_80_plan", item=...)      # Training recommendations
skills(action="t2_requirements", item=...)   # What needs level V for T2
skills(action="training_time", skill_list=...)  # Actual training duration
skills(action="get_multipliers", role=...)   # High-impact skills by role
```

### Fitting Tools
```
fitting(action="calculate_stats", eft=...)   # Full fit statistics via EOS
status()                                     # Check system health
```

**Fitting Validation Protocol:** When recommending ship fittings, ALWAYS:
1. Verify module names via `sde(action="item_info")` before building EFT
2. Validate the complete fit via `fitting(action="calculate_stats", eft=..., use_pilot_skills=true)`
...
market(action="orders", item=...)     # Order book details
```
...
- [ ] Did I validate the complete fit via EOS `fitting(action="calculate_stats")`?
...
5. ARIA verified with `sde(action="item_info", item="Drones")` and found the error

**Root cause:** Hallucinated skill effect from training data without verification.

**What should have happened:**
1. Before claiming what Drones skill does, call `sde(action="item_info", item="Drones")`
...
2. ARIA called `sde(action="blueprint_info")` which returned manufacturing data but not invention requirements
...
5. ARIA verified with `sde(action="search")` and confirmed the error
...
**What should have happened:**
1. Call `sde(action="blueprint_info", item="Drone Damage Amplifier II")` - got manufacturing data only
...
│ • sde(action="item_info")       │
│ • skill_* for training          │
│ • fitting(action="calculate_stats") │
│ • market_* for prices           │
└─────────────────────────────────┘
     │
     ▼
