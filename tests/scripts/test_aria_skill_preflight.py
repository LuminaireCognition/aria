"""Tests for .claude/scripts/aria-skill-preflight.py.
...
spec = importlib.util.spec_from_file_location("aria_skill_preflight", SCRIPT_PATH)
skill_preflight = importlib.util.module_from_spec(spec)
spec.loader.exec_module(skill_preflight)
...
def test_resolve_returns_id_and_directory(tmp_path: Path) -> None:
    ...
    char_id, directory = skill_preflight.resolve_active_pilot(tmp_path)
