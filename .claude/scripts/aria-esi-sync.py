...
def get_active_pilot_id(project_root: Path) -> str:
    ...
    if creds_dir.exists():
        cred_files = sorted(
            p for p in creds_dir.glob("*.json") if p.is_file() and re.fullmatch(r"[0-9]+", p.stem)
        )
        if len(cred_files) > 1:
            raise ValueError(
                "Multiple credential files found in userdata/credentials/: "
                + ", ".join(p.name for p in cred_files)
            )
        if cred_files:
...
def find_pilot_directory(project_root: Path, pilot_id: str) -> Path:
    ...
    matches = sorted(p for p in pilots_dir.glob(f"{pilot_id}_*") if p.is_dir())
    if len(matches) > 1:
        raise ValueError(
...
def load_credentials(project_root: Path, pilot_id: str) -> dict:
    ...
    creds_path = project_root / "userdata" / "credentials" / f"{pilot_id}.json"
