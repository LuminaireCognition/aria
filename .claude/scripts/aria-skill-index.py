...
def validate_skills(skills: list[dict[str, Any]], strict: bool = False) -> tuple[bool, dict]:
    ...
    for skill in skills:
        ...
        if issues:
            ...
            skill_entry["missing_required"] = [
                f.replace("Missing required field: ", "") for f in issues
            ]
    ...
    print("\n" + "=" * 60)
    print(
        f"Summary: {compliance_report['compliant']} compliant, "
...
def check_staleness(index: dict[str, Any], index_file: Path) -> bool:
    ...
    for name in gen_skills.keys() & ext_skills.keys():
        if json.dumps(gen_skills[name], sort_keys=True) != json.dumps(
            ext_skills[name], sort_keys=True
        ):
