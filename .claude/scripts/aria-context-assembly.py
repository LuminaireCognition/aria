...
def get_active_pilot_id(project_root: Path) -> str:
    ...
    3. Auto-detect from a single pilot in userdata/pilots/
    """
    ...
    # Fallback: check canonical pilots directory for a single numeric pilot
    pilots_dir = project_root / "userdata" / "pilots"
    if pilots_dir.exists():
        pilot_dirs = [
            ...
            and re.fullmatch(r"[0-9]+", d.name.split("_", 1)[0])
        ]
        if len(pilot_dirs) == 1:
            return pilot_dirs[0].name.split("_", 1)[0]
        if len(pilot_dirs) > 1:
            raise ValueError(
                "Multiple pilot directories found and no active pilot selected: "
                + ", ".join(sorted(d.name for d in pilot_dirs))
            )
...
def find_pilot_directory(project_root: Path, pilot_id: str) -> Path:
    ...
    Checks in canonical location:
    1. userdata/pilots/{pilot_id}_*
    """
    userdata_pilots = project_root / "userdata" / "pilots"
    if userdata_pilots.exists():
        matches = sorted(p for p in userdata_pilots.glob(f"{pilot_id}_*") if p.is_dir())
        if len(matches) > 1:
            raise ValueError(
...
def show_status(project_root: Path, pilot_id: str, as_json: bool = False) -> int:
...
def main():
    ...
    if args.status:
        status_code = show_status(project_root, pilot_id, args.json)
        if status_code != 0:
