...
def pilot_slug(name: str) -> str:
    slug = (name or "").lower().replace(" ", "_")
    slug = re.sub(r"[^a-z0-9_]", "", slug)[:32]
    return slug or "pilot"


def _acquire_lock(lockdir: Path) -> None:
    try:
        lockdir.mkdir()
        return
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise
    if lockdir.exists():
        lock_age = time.time() - lockdir.stat().st_mtime
        if lock_age > 300:
            ...
            return
    raise RuntimeError(
        f"Error: Lock exists at {lockdir}. If no other ARIA process is running, remove it:\n"
        f"  rmdir {lockdir}"
    )
...
def _load_json_or_default(path: Path, default: dict) -> dict:
    ...
    try:
        ...
        path.rename(backup)
        print_aria(f"Recovered corrupt JSON at {path.name}; backup saved to {backup.name}.")
        return default


def resolve_pilot_directory(
    project_root: Path, character_id: int, character_name: str, force: bool = False
) -> str:
    pilots_root = project_root / "userdata" / "pilots"
    pilots_root.mkdir(parents=True, exist_ok=True)
    placeholders = sorted(p for p in pilots_root.glob("0_*") if p.is_dir())

    if len(placeholders) > 1:
        names = "\n".join(f"  {p.name}/" for p in placeholders)
        raise RuntimeError(
            "Multiple placeholder directories found in userdata/pilots/:\n"
            f"{names}\nCannot determine which to adopt. Remove all but one and re-run."
        )

    if placeholders:
        placeholder = placeholders[0]
        old_slug = placeholder.name.split("_", 1)[1] if "_" in placeholder.name else "pilot"
        if not old_slug:
            old_slug = "pilot"
        adopted = f"{character_id}_{old_slug}"
        target = pilots_root / adopted
        if target.exists():
            raise RuntimeError(
                f"Target directory already exists: {target}\n"
                f"Cannot rename placeholder directory '{placeholder.name}/'."
            )
        ...
        populated = any((placeholder / rel).exists() for rel in canonical_files)
        if populated and not force:
            print_aria(
                f"Adopting placeholder directory '{placeholder.name}/' -> "
                f"'userdata/pilots/{adopted}/'."
            )
            print("Contains existing profile data and may belong to another character identity.")
            choice = input(f"Adopt this directory for {character_name}? [y/N]: ").strip().lower()
            if choice != "y":
                raise RuntimeError("Aborted placeholder adoption.")

        placeholder.rename(target)
        print_aria(f"Adopted placeholder directory '{placeholder.name}/' -> '{target.name}/'.")
        return target.name

    registry_path = pilots_root / "_registry.json"
    registry = _load_json_or_default(registry_path, {"schema_version": "1.0", "pilots": []})
    pilots = registry.get("pilots", [])
    existing = next(
        (p for p in pilots if str(p.get("character_id")) == str(character_id)),
        None,
    )
    if existing:
        directory_name = str(existing.get("directory", "")).strip()
        if not directory_name:
            raise RuntimeError(
                f"Pilot {character_id} already exists in _registry.json but has no directory value."
            )
        if "/" in directory_name or "\\" in directory_name:
            raise RuntimeError(
                f"Pilot {character_id} has invalid directory value in _registry.json: {directory_name}"
            )
        directory_path = pilots_root / directory_name
        if not directory_path.is_dir():
            raise RuntimeError(
                f"Pilot {character_id} exists in _registry.json but directory is missing: "
                f"userdata/pilots/{directory_name}/"
            )
        return directory_name

    directory_name = f"{character_id}_{pilot_slug(character_name)}"
    (pilots_root / directory_name).mkdir(parents=True, exist_ok=True)
    return directory_name


def update_pilot_registry(
    project_root: Path, character_id: int, character_name: str, directory_name: str
) -> None:
    registry_path = project_root / "userdata" / "pilots" / "_registry.json"
    registry = _load_json_or_default(registry_path, {"schema_version": "1.0", "pilots": []})
    pilots = registry.setdefault("pilots", [])
    now = utc_now_iso()
    ...
    if existing:
        ...
        existing["directory"] = directory_name
        existing.setdefault("faction", "Unknown")
        existing.setdefault("account_tag", "main")
...
def update_config(project_root: Path, character_id: int) -> None:
    config_path = project_root / "userdata" / "config.json"
    config = _load_json_or_default(config_path, {})
...
def save_credentials_secure(creds_path: Path, credentials: dict, use_keyring: bool = True):
    ...
    if use_keyring and is_keyring_enabled() and character_id:
        ...
        if keyring_saved:
            print_aria(
                f"✓ Credentials stored securely in system keyring (Tier II: {KEYRING_BACKEND})"
            )
...
""")
    ...
    credentials = {
        ...
        "_created": datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%SZ"),
    ...
    if creds_path.exists():
        ...
        if overwrite != "y":
            ...
            print_aria(f"Credentials updated: userdata/credentials/{character_id}.json")
    else:
        save_credentials_secure(creds_path, credentials, use_keyring=use_keyring)
        print_aria(f"Credentials saved: userdata/credentials/{character_id}.json")

    try:
        directory_name = resolve_pilot_directory(
            project_root, character_id, character_name, force=args.force
        )
    except RuntimeError as e:
        print_aria(f"Error: {e}")
        sys.exit(1)
    update_pilot_registry(project_root, character_id, character_name, directory_name)
...
NEXT STEPS:
  ...
  2. Set up automatic token refresh (recommended):
     ...
     LINUX/MACOS/WSL2 (crontab):
       crontab -e
       # Add: */15 * * * * cd {project_root} && .claude/scripts/aria-refresh --quiet

     Windows users: use crontab inside your WSL2 environment (same as Linux).
