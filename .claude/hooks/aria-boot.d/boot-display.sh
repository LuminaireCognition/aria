...
output_json_context() {
    ...
    # Build ESI reason JSON value
    local esi_reason_json="null"
    if [ -n "$ESI_REASON" ]; then
        local escaped_reason="${ESI_REASON//\"/\\\"}"
        esi_reason_json="\"$escaped_reason\""
    fi
